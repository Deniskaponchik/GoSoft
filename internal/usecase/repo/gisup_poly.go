...
func (gr *Repo) UpdateMapsToDBerr(polyMap map[string]entity.PolyStruct) (err error) {
	...
	for k, v := range polyMap {
		...
		queries = append(queries, "UPDATE "+gr.repoMySql.DataBase+".poly SET srid = '"+v.SrID+"', comment = "+strconv.Itoa(int(v.Comment))+" WHERE mac = '"+k+"';")
	}
...
func (gr *Repo) DownloadMapFromDBvcsErr(marker int) (map[string]entity.PolyStruct, error) {
	...
	for myError != 0 {
		if db, errSqlOpen := sql.Open("mysql", gr.repoMySql.DataSource); errSqlOpen == nil {
			...
			if errDBping == nil {
				defer db.Close() // defer the close till after the main function has finished
				queryAfter := "SELECT * FROM " + gr.repoMySql.DataBase + ".poly"
