...
func (gr *Repo) UpdateDbAp(mapAp map[string]*entity.Ap) (err error) {
	...
	b1.WriteString("REPLACE INTO " + gr.repoMySql.DataBase + ".ap" + " VALUES ")
...
func (gr *Repo) Download2MapFromDBaps() (map[string]*entity.Ap, map[string]*entity.Ap, error) {
	...
	for myError != 0 {
		...
		if db, errSqlOpen := sql.Open("mysql", gr.repoMySql.DataSource); errSqlOpen == nil {
			...
			if errDBping == nil {
				...
				//queryAfter := "SELECT * FROM " + ur.databaseITsup + ".ap" // WHERE controller = " + strconv.Itoa(int(ur.controller))
				queryAfter := "SELECT * FROM " + gr.repoMySql.DataBase + ".ap"
...
//func (gr *Repo) DownloadMapFromDBapsErr() (map[string]*entity.Ap, error) {
	...
	for myError != 0 {
		...
		if db, errSqlOpen := sql.Open("mysql", gr.repoMySql.DataSource); errSqlOpen == nil {
			...
			if errDBping == nil {
				...
				//queryAfter := "SELECT * FROM " + ur.databaseITsup + ".ap WHERE controller = " + strconv.Itoa(int(ur.controller))
				queryAfter := "SELECT * FROM " + gr.repoMySql.DataBase + ".ap WHERE controller = " + strconv.Itoa(int(ur.controller))
				log.Println(queryAfter)
