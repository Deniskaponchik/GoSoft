...
func (gr *Repo) UpdateDbAnomaly(mac_Anomaly map[string]*entity.Anomaly) (err error) {
	...
	b1.WriteString("INSERT INTO  " + gr.repoMySql.DataBase + ".anomaly VALUES ")
	countB1 := 0
...
func (gr *Repo) UpdateDbClient(mac_Client map[string]*entity.Client) (err error) {
	//bdCntrl := strconv.Itoa(int(ur.controller)) //bdController))
	var lenMap int
	var count int
	...
	b1.WriteString("REPLACE INTO " + gr.repoMySql.DataBase + ".client" + " VALUES ")
...
func (gr *Repo) DownloadMacMapsClientApWithAnomaly(macClient map[string]*entity.Client, macAp map[string]*entity.Ap, beforeDays string, timeNow time.Time) (err error) {
	...
	for myError != 0 {
		...
		if db, errSqlOpen := sql.Open("mysql", gr.repoMySql.DataSource); errSqlOpen == nil {
			...
			if errDBping == nil {
				...
				queryAfter := "SELECT * FROM " + gr.repoMySql.DataBase + ".anomaly WHERE date_hour >= '" + beforeDays + "' AND exception = 0 order by date_hour"
...
/*С использованием номера контроллера. УСТАРЕЛО
...
func (gr *Repo) DownloadClientsWithAnomalySlice(mac_Client map[string]*entity.Client, beforeDays string, timeNow time.Time) (err error) {
	...
	for myError != 0 {
		...
		if db, errSqlOpen := sql.Open("mysql", gr.repoMySql.DataSource); errSqlOpen == nil {
			...
			if errDBping == nil {
				...
				//queryAfter := "SELECT * FROM it_support_db.anomalies WHERE controller = " + strconv.Itoa(int(bdController))
				//queryAfter := "SELECT * FROM " + ur.databaseITsup + ".anomaly WHERE date_hour >= '" + beforeDays + "' AND controller = " + strconv.Itoa(int(ur.controller)) + " AND exception = 0 order by date_hour DESC"
				queryAfter := "SELECT * FROM " + gr.repoMySql.DataBase + ".anomaly WHERE date_hour >= '" +
					beforeDays + "' AND controller = " + strconv.Itoa(int(ur.controller)) + " AND exception = 0 order by date_hour DESC"
				log.Println(queryAfter)
...
func (gr *Repo) Download2MapFromDBclient() (map[string]*entity.Client, map[string]*entity.Client, error) {
	...
	for myError != 0 {
		...
		if db, errSqlOpen := sql.Open("mysql", gr.repoMySql.DataSource); errSqlOpen == nil {
			...
			if errDBping == nil {
				...
				queryAfter := "SELECT * FROM " + gr.repoMySql.DataBase + ".client ORDER BY modified" //Из-за случая, когда меняют ноутбук, но оставляют старое имя
				log.Println(queryAfter)
...
func (gr *Repo) GetLoginPCerr(client *entity.Client) (err error) { //(entity.Client, error) {
	...
	for myError != 0 {
		...
		if db, errSqlOpen := sql.Open("mysql", gr.repoGlpi.DataSource); errSqlOpen == nil {
			errDBping := db.Ping()
			if errDBping == nil {
				defer db.Close() // defer the close till after the main function has finished
				...
				queryAfter := "SELECT contact FROM " + gr.repoGlpi.DataBase + ".glpi_computers where name = ? ORDER BY date_mod DESC"
