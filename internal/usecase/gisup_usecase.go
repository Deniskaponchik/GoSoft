...
type GisupUseCase struct {
	repo GisupRepo //interface. НЕ ИСПОЛЬЗОВАТЬ *
	soap GisupSoap //interface. НЕ ИСПОЛЬЗОВАТЬ *
	rmq  GisupRmq  //interface. НЕ ИСПОЛЬЗОВАТЬ *
	c3po GisupC3po //interface. UnifiRestOut

	ldapt2 Authentication //interface
	jwtTok Authorization  //interface

	/*Остаются сущностями Unifi
...
// uiRostov Ui, uiNovosib Ui, c3po UnifiRestOut,
func NewGisupUC(repo GisupRepo, rmq GisupRmq, soap GisupSoap,
			...
			if lenAnomStructSlice > 9 {
				...
				if len(anomalyTempMap) > 9 { //если больше 9 дней с аномалями
					...
					ticket.Description = "На ноутбуке:" + "\n" +
						...
						"https://wiki.tele2.ru/pages/viewpage.action?pageId=168680976#id-[HelpdeskIT]SystemMonitoring-Аномалии" + "\n" +
						"" + "\n" +
						"Не нужно закрывать обращение, если кол-во дней с аномалиями за последние 30 дн. больше 9" + "\n" +
