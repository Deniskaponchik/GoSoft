...
func NewPoly(r PolyRepo, a PolyWebApi, n PolyNetDial, s PolySoap, everyCode map[int]bool, restartHour int) *PolyUseCase {
	return &PolyUseCase{
...
func (puc *PolyUseCase) InfinityPolyProcessing() error {
	...
	for true { //зацикливаем навечно
		...
		if timeNow.Minute() != 0 && everyCode[timeNow.Minute()] && timeNow.Minute() != count20minute {
			...
			//Перезагрузка
			if timeNow.Hour() == puc.restartHour && reboot == 0 { //Было 7 часов
			...
			if timeNow.Hour() == puc.restartHour+1 { //стояло 8
				reboot = 0
			}
